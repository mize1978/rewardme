...
| 📧 | **お手紙** — リボンちゃんから届くゲーム内メール | 13通・複数トリガー |
...
> - Turbo + Stimulus だけでゲームUIをフルスクラッチ実装（バンドラーなし）
> - 18種類の部屋テーマ × 3色キャラクターを CSS変数1つで切り替え
> - Devise を使わず、認証からゲームロジックまで自作
...
| # | 工夫 | 効果 |
|---|---|---|
| 1 | `completed_count` 1カラムから EXP・レベル・進化ステージをすべて計算 | バランス調整をコード変更だけで全ユーザーに即時反映。DBマイグレーション不要 |
| 2 | CSS カスタムプロパティで 18テーマを管理 | 新テーマの追加は変数ブロック1つだけ。既存 CSS は1行も変更不要 |
| 3 | セリフ 54パターンを Stimulus コントローラーに集約 | サーバー通信ゼロで即時動作。部屋 × キャラ色の組み合わせをすべてクライアント完結 |
...
**得られたメリット**：13通すべてでマイグレーション不要。新しい手紙の追加もコード追記だけで完結し、DBの状態を意識せず管理できる。
...
このアプリは「ここで終わり」ではなく、今後も継続して育てていく予定です。

| | 展望 |
|---|---|
| 📱 | Flutter版モバイルアプリ（iOS / Android） |
| 🔔 | プッシュ通知によるタスクリマインダー |
| 📊 | 習慣データの可視化・継続率分析ダッシュボード |
| 🤖 | AIによるタスク提案・習慣アドバイス機能 |
| 🌐 | 多言語対応（英語・中国語） |
